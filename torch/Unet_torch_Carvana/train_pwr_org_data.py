...
##### Local shifting ####
    ...
    # val_loader = DataLoader(val_set, shuffle=False, drop_last=True, batch_size=1000, num_workers=4, pin_memory=True)
    ...
    for epoch in range(1, epochs+1):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='maps') as pbar:
            for batch in train_loader:
                # print('train_x shape: ', train_x.shape);raise
                # print('batch: ', batch[0].shape, batch[1].shape);raise
                # images = train_x
                # true_masks = train_y
                ...
                print(f'train loss: {loss.item():.3e} step: {global_step} epoch: {epoch} lr: {cur_lr}')
...
if __name__ == '__main__':
    ...
    if args.load and args.mode == 'train':
        ...
        if torch.cuda.device_count() > 1:
          print("Available GPU: ", torch.cuda.device_count(), "GPUs!\n")
          # dim = 0 [30, xxx] -> [10, ...], [10, ...], [10, ...] on 3 GPUs
          # net = nn.DataParallel(net)
          print(f'Using: {device_list}')
          net = nn.DataParallel(net, device_ids = device_list)
