...
if train_mode:
    ...
    model.fit(input_data, output_data, batch_size=512//1, epochs=5000, verbose=True,
        ...
        val_pred = model.predict(val_data_n.reshape((val_data.shape[0] , subdomain_size[0],
                                                        subdomain_size[1], num_conditions)), batch_size=2048)

        val_pred = val_pred.reshape(val_data.shape[0] // total_subdomains, total_subdomains, subdomain_size[0],
                                    subdomain_size[1], num_conditions)

        val_data = (val_data) * (tr_mx - tr_mn) + tr_mn
        val_pred = (val_pred) * (tr_mx - tr_mn) + tr_mn


        val_data = val_data.reshape(val_data.shape[0] // total_subdomains, total_subdomains, *subdomain_size, num_conditions)
        ...
        for iv in range(num_vars):
            ...
            # raise
            file_plot = "./plots/generative_results/"+variable_name+"/variable_"+str(iv)+"/"
            ...
            for j in range(100):
                ...
                mae_err.append(np.mean(data["Error"]))
                contours_2d(data, filename)
            print('MAE on all test cases: ', sum(mae_err)/len(mae_err))
